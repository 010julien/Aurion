...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      await Promise.all([loadDevices(), loadSecurity(), loadEnergy()]);
      setLoading(false);
    };
    loadData();
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Toggle enabled={settings.awayMode} onChange={toggleAwayMode} />
        </div>
      </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-yellow-800">
                Vous avez {unacknowledgedAlerts} alerte(s) de sécurité non
                acquittée(s).
              </p>
            </div>
            <Button
          ...
          {Object.entries(devicesByRoom).map(([room, roomDevices]) => (
            <Card key={room} hover>
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge
                  variant={
                    roomDevices.some((d) => d.status) ? "success" : "default"
                  }
                >
                  {roomDevices.filter((d) => d.status).length}/
                  {roomDevices.length} actifs
                </Badge>
              </div>
